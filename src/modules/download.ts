...
export async function downloadPackage(downloadUrl: string, fileName: string): Promise<DownloadPackageData | null> {
    try {
        // 配置下载超时
        ...
        try {
            ...
            if ((error as Error).name === "AbortError") {
                message(i18n.downloadTimeout, true);
                return null;
            }
            const msg = error instanceof Error ? error.message : String(error);
            message(i18n.downloadFailed.replace("{error}", msg), true);
            return null;
        }
        ...
        // 校验 HTTP 响应状态
        if (!response.ok) {
            message(i18n.downloadFailed.replace("{error}", `HTTP ${response.status}`), true);
            return null;
        }
        ...
        if (!validatePackageZipFile(uint8Array, fileName)) {
            message(i18n.fileValidationFailed, true);
            return null;
        }
        ...
        if (!packageName) {
            message(i18n.packageNameFromUrlFailed, true);
            return null;
        }
